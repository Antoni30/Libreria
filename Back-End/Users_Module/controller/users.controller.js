...
export const  getUsers = async (req, res)=>{
    try {
        ...
        const users= rows.map(row => new User(row.id_user_role,row.user_fullname,row.user_email,row.user_password,row.user_phone,row.user_registration_date,row.id_firebase,row.id_user))
...
export const postUser = async (req,res)=>{
    try {
        ...
        const { id_user_role,user_fullname,user_email,user_password,user_phone,id_firebase} = req.body;  
        ...
        await pool.query("Call insert_data_user($1,$2,$3,$4,$5,$6)",[newUser.getUserRole(),newUser.getUserFullName(),newUser.getUserEmail(),newUser.getUserPassword(),newUser.getUserPhone(),id_firebase]);
        res.status(200).json({
...
export const getUserByID = async (req, res) => {
    try {
        ...
        const { rows } = await pool.query("SELECT * FROM get_user_by_id($1)", [id]);
        
        if (rows.length === 0) {
            return res.status(404).json({ message: "Usuario no encontrado" });
        }

        const user = new User(rows[0].id_user_role, rows[0].user_fullname, rows[0].user_email, rows[0].user_password, rows[0].user_phone, rows[0].user_registration_date, rows[0].id_firebase, rows[0].id_user);
...
export const getUserByFirebaseID = async (req, res) => {
    try {
        const { id } = req.params; // Obtener el ID de Firebase desde los parámetros de la URL
        const { rows } = await pool.query("SELECT * FROM get_user_by_firebase_id($1)", [id]);
